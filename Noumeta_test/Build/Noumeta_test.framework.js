...
var ASM_CONSTS = {
  3391948: function() {Module['emscripten_get_now_backup'] = performance.now;},  
 3392003: function($0) {performance.now = function() { return $0; };},  
 3392051: function($0) {performance.now = function() { return $0; };},  
 3392099: function() {performance.now = Module['emscripten_get_now_backup'];},  
 3392154: function() {return Module.webglContextAttributes.premultipliedAlpha;},  
 3392215: function() {return Module.webglContextAttributes.preserveDrawingBuffer;},  
 3392279: function() {return Module.webglContextAttributes.powerPreference;}
};
  ...
  function _CallExternalJS() {
          console.log("Unityから呼ばれた: CallExternalJS");
  
          // 実際の処理は外部JSに任せる
          if (typeof joinAgoraChannel === "function") {
              joinAgoraChannel();
          } else {
  ...
  function _FocusInput(inputIdPtr) {
      ...
      if (!el) {
        console.warn("FocusInput: element not found: " + inputId);
        return;
      }
  
      // focus イベントは一度だけ登録
      if (!el.hasFocusListener) {
        el.addEventListener('focus', (e) => {
          e.preventDefault();
          // 強制スクロール補正
          setTimeout(() => window.scrollTo(0, 0), 0);
        });
        el.hasFocusListener = true;
      }
  
      // Unity側から呼ばれたときだけフォーカス
      if (!el.dataset.focused || el.dataset.focused === "false") {
